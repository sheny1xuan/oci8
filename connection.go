...
func (conn *Conn) PrepareContext(ctx context.Context, query string) (driver.Stmt, error) {
	...
	if conn.stmtCacheSize == 0 {
		...
		return &Stmt{conn: conn, stmt: *stmt, ctx: ctx, releaseMode: C.OCI_DEFAULT, sql: query}, nil
	}
	...
	return &Stmt{conn: conn, stmt: *stmt, ctx: ctx, releaseMode: C.OCI_DEFAULT, cacheKey: query, sql: query}, nil
}
...
func (mc *Conn) execAlways(query string, args []driver.Value) (driver.Result, error) {
	...
	return s.execForSeata(args)
}

func (mc *Conn) prepareQuery(query string, args []driver.Value) (driver.Rows, error) {
	stmt, err := mc.Prepare(query)
	if err != nil {
		return nil, err
	}
	s := stmt.(*Stmt)
	return s.localQuery(args)
}
