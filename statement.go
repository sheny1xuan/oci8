...
func (stmt *Stmt) Query(args []driver.Value) (driver.Rows, error) {
	...
	if stmt.conn.ctx != nil {
		parser := parser.New()
		mysqlStmt, err := oracle2Mysql(stmt.sql)
	...
	binds, err := stmt.bindValues(args, nil)
	if err != nil {
		return nil, err
	}
...
func (stmt *Stmt) Exec(args []driver.Value) (driver.Result, error) {
	...
	if stmt.conn.ctx != nil {
		parser := parser.New()
		// stmt.sql = "insert into test ( col1, col2 ) values ( ?, ?)"
		mysqlStmt, err := oracle2Mysql(stmt.sql)
...
func (stmt *Stmt) execForSeata(args []driver.Value) (driver.Result, error) {
	stmt.ctx = context.Background()
	binds, err := stmt.bindValues(args, nil)
	if err != nil {
		return nil, err
